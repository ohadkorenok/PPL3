...
var makeASTCexp = function (exp) {
    return L4_ast_1.isAppExp(exp) ? makeTree(exp.tag, [makeASTCexp(exp.rator),
        makeTree(":", [].concat(ramda_1.map(makeASTCexp, exp.rands)), [].concat(ramda_1.map(function (x) {
        ...
        L4_ast_1.isIfExp(exp) ? makeTree(exp.tag, [].concat(ramda_1.map(makeASTCexp, [exp.test, exp.then, exp.alt])), ["test", "then", "alt"]) :
            L4_ast_1.isProcExp(exp) ? makeTree(exp.tag, [].concat(ramda_1.map(makeASTVarDecl, exp.args)).concat(ramda_1.map(makeASTCexp, exp.body)), ["args", "body"]) :
                L4_ast_1.isAtomicExp(exp) ? makeAstAtomicExp(exp) :
                    L4_ast_1.isLitExp(exp) ? makeTree(exp.tag, [makeAstSexp(exp.val)], ["val"]) :
                        L4_ast_1.isLetExp(exp) ? makeTree(exp.tag, [
                            makeTree(":", [].concat(ramda_1.map(function (x) {
                                return makeTree(x.tag, [makeASTVarDecl(x.var), makeASTCexp(x.val)], ["var, val"]);
                            }, exp.bindings)), [].concat(ramda_1.map(function (x) {
                                return exp.bindings.indexOf(x).toString();
                            }, exp.bindings))),
                            makeTree("body", ramda_1.map(makeASTCexp, exp.body), [].concat(ramda_1.map(function (x) {
